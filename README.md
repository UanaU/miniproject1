...
- **관리비 비교** (`/fees/compare`) — 청구년월과 **비교 항목 드롭다운**(전월 비교 / 작년 동월 비교 /
  같은 평수 비교 / 같은 평수+가구원수 비교)을 선택하면, 그 비교 기준에 대해 **총관리비/전기세/수도세/가스비**
  4개 막대그래프가 한 화면에 동시에 표시된다. "이번달"과 비교 대상 막대는 서로 다른 색으로 구분된다
...
- **막대그래프**(관리비 비교의 이번달 vs 비교 대상): `computeStepDomain`/`buildStepTicks`로 Y축 눈금을
  **정확히 500원 간격**으로 고정해서 그린다 (모든 눈금을 생략 없이 표시)
