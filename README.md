...
- **관리비 비교** (`/fees/compare`) — 청구년월을 고른 뒤, 비교 기준을 큰 탭 두 가지 중에서 고른다
  ...
  **이번달 + 그 탭의 비교 대상 2개, 총 3개의 막대**가 서로 다른 색으로 표시된다 (예: "내관리비에서 비교" 탭의
  ...
  화면 하단에는 AI 원인분석 카드가 있어, 이번달/전월/같은평수평균/같은평수+가구원수평균 데이터를 근거로
  Gemini가 증가 원인과 절약팁을 알려준다 (API 키가 없거나 실패하면 안내 문구로 대체)
...
- `POST /api/managementfee/ai-analysis` — 이번달/전월/같은평수평균/같은평수+가구원수평균 데이터를 보내면
  Gemini가 원인 요약·항목별원인·절약팁을 반환 (관리비 비교 화면의 AI 원인분석 카드에서 사용)
