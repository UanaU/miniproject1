...
최소~최대 범위 근처로 좁혀서(줌인해서)** 그려진다 (`frontend/src/utils/chart.js`의 `computeTightDomain`).
값 변화나 이번달 대비 비교 차이가 실제 금액 차이보다 시각적으로 더 뚜렷하게 보이도록 하기 위함이며,
축 범위가 좁아 눈금 라벨이 겹칠 수 있는 구간에서는 자동으로 소수점 한 자리(`formatAxisTick`)를 붙여 표시한다.
