...
- **로그인** (`/login`) — 아이디/비밀번호 로그인, 하단에 회원가입 링크. 로그인 성공 시 보유한 관리비
  레코드가 하나도 없는 신규 가입자는 **관리비 등록** 화면으로, 그 외에는 **마이페이지**로 이동한다
...
- **관리비 조회** (`/fees`) — 청구년월 선택, 항목별 관리비 표시. 좌측의 **합계금액/전기세/수도세/가스비** 항목을
  클릭하면 우측에 해당 항목의 **선택한 청구년월이 속한 연도(1~12월)** 추이가 꺾은선 그래프로 표시된다
...
- **관리비 비교** (`/fees/compare`) — 청구년월과 함께 **비교 항목 드롭다운**(전월 비교 / 작년 동월 비교 /
  같은 평수 비교 / 같은 평수+가구원수 비교)에서 선택한 막대그래프 1개를 보여준다

로그인 후에는 화면 우측 상단에 **마이페이지** / **내관리비**(하위 메뉴: 관리비 조회·관리비 등록·관리비 비교)
